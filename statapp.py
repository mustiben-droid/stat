...
if uploaded_file:
    # 1. טעינת האקסל - הגדרת שורה 2 (אינדקס 1) ככותרות (Header)
    # זה מבטיח ששמות כמו 'Major' ו-'Pre Score' יהיו שמות העמודות הרשמיים.
    df = pd.read_excel(uploaded_file, header=1)
    
    # 2. ניקוי שמות העמודות וטיפול בכפילויות/ריקים
    new_columns = []
    for i, col in enumerate(df.columns):
        clean_name = str(col).strip()
        # אם השם ריק או שזה 'Unnamed', ניתן לו שם גנרי כדי למנוע קריסה
        if clean_name == "" or "Unnamed" in clean_name or "None" in clean_name:
            new_columns.append(f"Var_{i}")
        else:
            new_columns.append(clean_name)
    df.columns = new_columns

    # 3. ניסיון המרה אוטומטי למספרים
    for col in df.columns:
        df[col] = pd.to_numeric(df[col], errors='ignore')

    st.success(f"הקובץ נטען בהצלחה! זוהו {len(df.columns)} משתנים.")
